...
func (con GoodsTypeAttributeController) Index(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "admin/goodsTypeAttribute.html", gin.H{
		"cateId":                 cateId,
...
func (con GoodsTypeAttributeController) Add(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "admin/goodsTypeAttributeAdd.html", gin.H{
...
func (con GoodsTypeAttributeController) DoAdd(c *gin.Context) {
	...
	if title == "" {
		con.Error(c, "商品类型属性名称不能为空", "/admin/goodsTypeAttribute/add?cate_id="+c.PostForm("cate_id"))
		return
	}

	if err3 != nil {
		con.Error(c, "排序值不对", "/admin/goodsTypeAttribute/add?cate_id="+c.PostForm("cate_id"))
		return
	}
	...
	if err != nil {
		con.Error(c, "增加商品类型属性失败 请重试", "/admin/goodsTypeAttribute/add?cate_id="+c.PostForm("cate_id"))
	} else {
		con.Success(c, "增加商品类型属性成功", "/admin/goodsTypeAttribute?id="+c.PostForm("cate_id"))
	}
...
func (con GoodsTypeAttributeController) Edit(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "admin/goodsTypeAttributeEdit.html", gin.H{
...
func (con GoodsTypeAttributeController) DoEdit(c *gin.Context) {
	...
	if title == "" {
		con.Error(c, "商品类型属性名称不能为空", "/admin/goodsTypeAttribute/edit?id="+c.PostForm("id"))
		return
	}
	if err4 != nil {
		con.Error(c, "排序值不对", "/admin/goodsTypeAttribute/edit?id="+c.PostForm("id"))
		return
	}
	...
	if err != nil {
		con.Error(c, "修改数据失败", "/admin/goodsTypeAttribute/edit?id="+c.PostForm("id"))
		return
	}
	con.Success(c, "需改数据成功", "/admin/goodsTypeAttribute?id="+c.PostForm("cate_id"))
}
...
func (con GoodsTypeAttributeController) Delete(c *gin.Context) {
	...
	if err1 != nil {
		...
		con.Success(c, "删除数据成功", "/admin/goodsTypeAttribute?id="+c.Query("cate_id"))
	}
}
