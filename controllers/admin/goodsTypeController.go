...
func (con GoodsTypeController) Index(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "admin/goodsType.html", gin.H{
		"goodsTypeList": goodsTypeList,
	})

}
func (con GoodsTypeController) Add(c *gin.Context) {
	c.HTML(http.StatusOK, "admin/goodsTypeAdd.html", gin.H{})
}
...
func (con GoodsTypeController) Edit(c *gin.Context) {
	...
	if err != nil {
		...
		c.HTML(http.StatusOK, "admin/goodsTypeEdit.html", gin.H{
			"goodsType": goodsType,
		})
	}
...
func (con GoodsTypeController) DoEdit(c *gin.Context) {
	...
	if title == "" {
		con.Error(c, "商品类型的标题不能为空", "/admin/goodsType/edit?id="+c.PostForm("id"))
	}
	goodsType := models.GoodsType{Id: id}
	models.DB.Find(&goodsType)
	...
	if err3 != nil {
		con.Error(c, "修改数据失败", "/admin/goodsType/edit?id="+c.PostForm("id"))
	} else {
